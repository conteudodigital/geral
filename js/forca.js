...
function gameForca(canvasID, questions) {
	...
	var t_boneco = new createjs.Bitmap("resources/image/boneco_hit.png");
    ...
    var i;
	for(i=0;i<6;i++){
	    boneco[i] = new createjs.Bitmap("resources/image/p"+i+".png");
	...
	btinicia.on("click", function() {
		...
		forkaimg = new createjs.Bitmap("resources/image/forka.png");
	...
	tente = new createjs.Bitmap("resources/image/tentenovamente.png");
	...
	help = new createjs.Bitmap("resources/image/help.png");
...
function montaFase(questions){
    ...
    for(i = 0; i < t.length; i++){
		hits[i] = new createjs.Bitmap("resources/image/hit.png");
	...
	for(i=0;i<26;i++){
	        letras[i] = new createjs.Bitmap("resources/image/l"+i+".png");
	        ...
	        letras[i].on("pressup", function (evt) {
			    ...
			    if(this.x<limite && this.y>520){
					...
					for(i=0;i<hits.length;i++){
						if(hits[i].n==this.n){
							...
							createjs.Tween.get(letra).to({scaleX:0.8,scaleY:0.8},300,createjs.Ease.backOut);
	        	...
	        	if(volta){
					this.pode=false;
				    var err = new createjs.Bitmap("resources/image/errado.png");
    ...
    console.log(questions[count][1]);
        ...
        if(extensao=='jpg' || extensao=='png'){
            var frase = new createjs.Bitmap("resources/image/"+questions[count][1]);
        frase.image.onload = function(){};
		content.addChild(frase);
        frase.x=questions[count][2];
        frase.y=questions[count][3];
        }else{
            var frase = caixaTexto(questions[count][1],33);
        frase.x=300;
        frase.y=380;	
        content.addChild(frase);
...
function criaLetra(texto,tam){
	...
	var c = new createjs.Bitmap("resources/image/l_vazia.png");
...
function verificaErros(questions){
	...
	if(i_acertos >= questions[count][0].length){
		...
		relogio.visible=false;
	    inicio1=false;
		var certo = new createjs.Bitmap("resources/image/certo.png");
