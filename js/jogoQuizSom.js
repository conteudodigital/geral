...
var contenthit;
var camadas=[];
var posX=[180,490,800,180,490,800,180,416,648,880];
var posY=[12,12,12,228,228,228,442,442,442,442];
var inicio1=false;
var btsom;
var btinicia;
var positivo;
...
function quizSomInicio(idCanvas,idFundo,idBtInicio) {
    var index;
    var t;
    for (index in sons) {
        t = sons[index];
	...
	btinicia.on("click", function() {
        ...
        if(typeof enunciado !== 'undefined'){
            var enun=new Audio(caminho+enunciado[0]);
            enun.play();
            frase = criaTitulo(enunciado[1],33);
            stage.addChild(frase);
            frase.x = -1280;
            frase.y = 350;
            createjs.Tween.get(frase).to({x:640},600,createjs.Ease.backOut).wait(enunciado[2]).to({x:2600},600).call(tocaSom);
...
function montaFase(){
    ...
    */
	...
	for(j=0;j<itens[count].opcoes.length;j++){
        ...
        content.addChild(bt);
		bt.x=bt.px=(1280/itens[count].opcoes.length)*j+imgLargura/2+offsetX;
		bt.y=bt.py=950;
		bt.pode=true;
		bt.n=t;
        bt.regX=imgLargura/2;
        bt.regY=imgAltura/2;
        console.log(imgLargura);
