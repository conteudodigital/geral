/*
-adicionada ascessao caso nao tenha feedback de respostas
v2 com imagens

*/
var AppTabuleiro=function(itens, idCanvas, idFundo, idBtiniciar,_btImg,_audioBt,_audioTit,_errosPermitidos=1){
	...
	function checkTime1(){
		var formatted = convertTime(Date.now() - startTime_fase);
	...
	function convertTime(miliseconds) {
		...
		segundos=totalSeconds - minutes * 60;
		if(seconds<10){
			seconds='0'+seconds;
		}
	...
	function fazPergunta(){
		startTime_fase = Date.now();
		updateTime1=true;
		updateTime2=true;
		relogio.gotoAndPlay("tempo1");
		relogio.visible=true;
		relogio.scaleX=relogio.scaleY=0.4;
		createjs.Tween.get(relogio).to({scaleX:1,scaleY:1,rotation:10}, 250,createjs.Ease.backOut).wait(350).to({rotation:-5}, 250,createjs.Ease.backOut);
		texto_tempo.x=1000;
		texto_tempo.y=580;
		...
		if(_btImg){
			...
			createjs.Tween.get(c_balao).to({y:0,scaleX:1,scaleY:1},500,createjs.Ease.backOut);
				...
				bt.on("mousedown", function() {
					if(clicavel){
						...
						if(this.n==resposta){
							...
							if(itens[ordem].pergunta[3]){
								createjs.Tween.get(this).to({scaleX:1,scaleY:1},400,createjs.Ease.backOut).wait(1000).call(
									function(){
										c_balao.removeAllChildren();

										var tit = new createjs.Bitmap(caminho+itens[ordem].pergunta[3]);
										tit.image.onload = function(){};
										tit.x=itens[ordem].pergunta[1];
										tit.y=itens[ordem].pergunta[2];
										c_balao.addChild(tit);
										var bt=new createjs.Bitmap(caminho+'btTabuleiroFechar.png');
										bt.image.onload = function(){};
										c_balao.addChild(bt);
										bt.x=1000;
										bt.y=50;
								...
								createjs.Tween.get(this).to({scaleX:1,scaleY:1},400,createjs.Ease.backOut).wait(2000).call(
									function(){
										segundosTotais+=segundos;
										console.log(segundosTotais);
										if(segundos<7){
			...
			c_balao.scaleX=c_balao.scaleY=0.15;
			createjs.Tween.get(c_balao).to({x:640,y:120,scaleX:1,scaleY:1},300,createjs.Ease.backOut);

			console.log(ordem);

			var tit = new createjs.Text(itens[ordem].pergunta, "bold 30px VAG Rounded BT", "#000000");
	...
	function caixaTexto(texto){
		var txt = new createjs.Text(texto, "bold 30px VAG Rounded BT", "#FF6600");
	...
	function animaIco(qual,b,c){
		...
		createjs.Tween.get(ico).to({scaleX:0.25,scaleY:0.25},200,createjs.Ease.quadOut);
	}
	function shuffle(a) {
	...
	function verificaFim(){
		...
		texto_errado.x=589;
		texto_errado.y=385;
		texto_tempo.text=segundosTotais+"s";
		texto_errado.text=i_erros;
		if(count>=movimento.length-1 && i_acertos>0){
			img=caminho+"gui_tabuleiro.png";
			continua=true;
			sons[2].play();
			
		}else{
			img=caminho+"tentetabuleiro.png";
			continua=true;
			sons[3].play();
			texto_tempo.x=-1000;
			texto_errado.x=-1000;
		...
		if(continua){
			...
			stage.addChild(bo);
			stage.addChild(texto_tempo);
			stage.addChild(texto_errado);
			createjs.Tween.get(bo).to({y:171},200,createjs.Ease.quadOut);
			bo.on("mousedown", function (evt) {
				stage.removeChild(this);
				stage.removeChild(texto_tempo);
				stage.removeChild(texto_errado);
				count=0;
				ordem=0;
				moveCasas=0;
				inicio1=true;
	...
	texto_tempo = new createjs.Text("0:00", "bold 50px VAG Rounded BT", "#000000");
	texto_tempo.x=634;
	texto_tempo.y=631;
	texto_tempo.textAlign = "center";

	texto_errado = new createjs.Text("0", "bold 50px VAG Rounded BT", "#000000");
	texto_errado.x=589;
	texto_errado.y=385;
