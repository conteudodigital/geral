	...
	*/
	...
	PANOLENS.Infospot.prototype.onClick = function ( event ) {
		...
		if ( this.element && this.getContainer() ) {
            ...
            if(event.target.mudar_cena_para){
                scope.dispatchEvent( { type : 'panolens-viewer-handler', method: 'setPanorama', data: event.target.mudar_cena_para } );
                console.log(this);
            }else{
	...
	PANOLENS.Viewer = function ( options ) {
		...
		this.OUTPUT_INFOSPOT_SHIFT = false;
	...
	PANOLENS.Viewer.prototype.outputInfospotPosition = function () {
		...
		if ( intersects.length > 0 ) {
            ...
            console.clear();
            var debugador='posicaoInicial:['+outputPosition.x+','+outputPosition.x+','+outputPosition.x+'],';
            console.info(debugador);

		}

	};
    /*modo edicao 
powered by:SAE digital*/
    PANOLENS.Viewer.prototype.outputInfospotPositionEdit = function () {

		var intersects,intersects_obj,intersect_entity, point, panoramaWorldPosition, outputPosition;

		intersects = this.raycaster.intersectObject( this.panorama, true );
        
        intersects_obj = this.raycaster.intersectObjects( this.panorama.children, true );
        intersect_entity = this.getConvertedIntersect( intersects_obj );
		...
		if ( intersects.length > 0 ) {
            ...
            if(modoEdicao_360){ 
                if(intersect_entity){
                    intersect_entity.position.set(outputPosition.x,outputPosition.y,outputPosition.z);
                    this.panorama.itens[intersect_entity.idOrdem].pos[0]=outputPosition.x;
                    this.panorama.itens[intersect_entity.idOrdem].pos[1]=outputPosition.y;
                    this.panorama.itens[intersect_entity.idOrdem].pos[2]=outputPosition.z;
                    criaDebug(this.panorama,intersect_entity.idOrdem);
    ...
    function criaDebug(pano,idOrdem){
        console.clear();

        var debugador='-------->Cena_'+(pano.idOrdem+1);
        debugador+='\n';
        debugador+='\n';
        debugador+='var itens_panorama'+(pano.idOrdem+1)+'=[';
        debugador+='\n';
        var i;
        var t;
        for(i=0;i<pano.itens.length;i++){
            debugador+='{';
            debugador+='imagem:"'+pano.itens[i].imagem+'",';
            debugador+='\n';
            debugador+='tamanho:'+pano.itens[i].tamanho+',';
            debugador+='\n';
            debugador+='pos:['+pano.itens[i].pos+'],';
            debugador+='\n';
            debugador+='div:"'+pano.itens[i].div+'",';
            debugador+='\n';
            debugador+='divOffsetY:'+pano.itens[i].divOffsetY+',';
            debugador+='\n';
            debugador+='muda_para_cena:'+pano.itens[i].muda_para_cena;
            
            if(i<pano.itens.length-1){
                debugador+='},';
                debugador+='\n';
            }else{
                debugador+='}];';
	...
	PANOLENS.Viewer.prototype.onTap = function ( event, type ) {
        ...
        if ( event.type !== 'mousedown' && PANOLENS.Utils.checkTouchSupported() || this.OUTPUT_INFOSPOT_SHIFT ) { 

			this.outputInfospotPositionEdit(); 
	...
	PANOLENS.Viewer.prototype.onKeyDown = function ( event ) {
        ...
        if ( this.options.output && this.options.output !== 'none' && event.key === 'Shift' ) {

			this.OUTPUT_INFOSPOT_SHIFT = true;
